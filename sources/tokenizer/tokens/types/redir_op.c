...
/*   Updated: 2017/06/09 13:56:11 by mo0ky            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void 	get_remaining_block(t_tokenizer *tokenizer, char *line)
{
	...
	//A FIXER
	//if (line < ptr_end && *(ptr_end - 1) == ' ') //chasnger condition par function qui teste si digit
	if (is_io_number(*ptr_begin, ptr_end))
...
static void	redirecting_oupout(t_tokenizer *tknizer, char *line)
{
	...
	{
		get_remaining_block(tknizer, line);
		create_token(&tknizer->token, ">", REDIR_OP);
	}
}
