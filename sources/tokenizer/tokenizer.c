...
/*   Updated: 2017/07/03 13:00:58 by jmoucade         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int analyze_block(t_tokenizer *tokenizer, char *line)
{
	...
	if (**ptr_end == ';' || **ptr_end == '|' || **ptr_end == '&')
	{
		if (*(*ptr_end + 1) == **ptr_end)
			printf("DOUBLONnnnnnnnnnnn\n");
		else
		{
...
int		tokenizer(t_tokenizer *tokenizer, char *line)
{
	...
	{
		if (is_special_char(*tokenizer->delimiter.end) || is_inhibitor_char(*tokenizer->delimiter.end))
		{
