...
/*   Updated: 2017/05/17 06:14:33 by jmoucade         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int is_arrow_keys(char *buff, t_editline *editline, t_history *h)
{
	if (!buff)
		return (0);
	if (editline->pos > 0 && IS_K_ARROW_LEFT(buff))
		return (k_arrow_left(&editline->pos));
	else if (editline->pos < (int)ft_strlen(editline->line) && IS_K_ARROW_RIGHT(buff))
...
int 	is_dels_keys(char *buff, char **line, int *pos, t_history *history)
{
	...
	if (*pos > 0 && IS_K_DELETE(buff))
		...
		return (k_suppr(line, *pos, history, ft_strlen(stock_data(NULL)->prompt) + 3));
	return (0);
}
